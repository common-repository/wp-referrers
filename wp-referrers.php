...
Description: This plugin shows the referrers to your blog
Author: Thomas Tsoi
Version: 1.01
Author URI: http://www.thomastsoi.com/
Plugin URI: http://www.thomastsoi.com/software/wp-referrers/
*/
...
function log_referrer() {
	...
	if (substr($referrer, 0, strlen($wpurl)) != $wpurl) {
		...
		if (ereg("^http://([^/]+)/.+", $referrer, $vars)) {
			...
			// check whether referrer already exists in the database
			$qryresult = $wpdb->query("SELECT id FROM $wp_referrers WHERE url = '$url' AND referrer = '$referrer'");

			if ($qryresult > 0) {
				// Update an existing referrer record
				$sql  = "UPDATE $wp_referrers";
				$sql .= " SET hits = hits + 1, last_time = NOW() ";
				$sql .= " WHERE url = '$url' AND referrer = '$referrer'";
				$wpdb->query($sql);
			}
			else {
				// Create a new referrer record
				$sql  = "INSERT INTO $wp_referrers SET";
				$sql .= " url = '$url',";
				$sql .= " referrer = '$referrer',";
				$sql .= " search_engine = $search_engine,";
...
function is_search_engine($referrer) {
	...
	return array($search_engine, $kwid);
}
